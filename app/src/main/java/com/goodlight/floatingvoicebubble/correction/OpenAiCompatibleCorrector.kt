...
class OpenAiCompatibleCorrector(private val endpoint: String, private val model: String, private val apiKey: String) : TextCorrector {
    override val id: String = "byok:$model"
    ...
    override fun correct(request: CorrectionRequest): String {
        ...
        val connection = (URL(endpoint).openConnection() as HttpURLConnection).apply {
            requestMethod = "POST"; connectTimeout = 10_000; readTimeout = 45_000; doOutput = true
        ...
        try {
            ...
            if (status !in 200..299) error("BYOK request failed: HTTP $status ${responseText.take(500).replace(Regex("\\s+"), " ")}")
            val response = JSONObject(responseText)
            val choice = response.optJSONArray("choices")?.optJSONObject(0) ?: error("BYOK response has no choices")
