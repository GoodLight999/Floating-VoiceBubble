    ...
    fun system(request: CorrectionRequest): String = buildString {
        ...
        when (request.preferences.lineBreakMode) {
            ...
            LineBreakMode.SMART -> appendLine(
                "- 話題・文意・列挙の区切りが明確な場所に1回の改行を入れる。RAWに句読点がなくても文意から境界を判断する。40文字程度以上の連続発話で複数文・複数論点があるなら、少なくとも1か所は改行する。短い単一文を無理に分割せず、短文ごとにも細切れにしない。空行は作らない。",
            )
            LineBreakMode.SMART_SPACED -> appendLine(
                "- 話題・文意・列挙の区切りが明確な場所に2回改行して1行分の空行を入れる。RAWに句読点がなくても文意から境界を判断する。40文字程度以上の連続発話で複数文・複数論点があるなら、少なくとも1か所は空行を入れる。短い単一文を無理に分割せず、短文ごとにも細切れにしない。",
            )
        }
        when {
